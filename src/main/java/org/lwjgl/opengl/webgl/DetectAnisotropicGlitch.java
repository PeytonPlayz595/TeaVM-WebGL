...
public class DetectAnisotropicGlitch {
	...
	public static boolean detect() {
		...
		if(ctx.getExtension("EXT_texture_filter_anisotropic") != null) {
			...
			WebGLShader vsh = ctx.createShader(RealOpenGLEnums.GL_VERTEX_SHADER);
			ctx.shaderSource(vsh, vshSrc);
			ctx.compileShader(vsh);
			
			if(!ctx.getShaderParameterb(vsh, RealOpenGLEnums.GL_COMPILE_STATUS)) {
			...
			WebGLShader fsh = ctx.createShader(RealOpenGLEnums.GL_FRAGMENT_SHADER);
			ctx.shaderSource(fsh, fshSrc);
			ctx.compileShader(fsh);
			
			if(!ctx.getShaderParameterb(fsh, RealOpenGLEnums.GL_COMPILE_STATUS)) {
			...
			if(!ctx.getProgramParameterb(pr, RealOpenGLEnums.GL_LINK_STATUS)) {
			...
			ctx.bindTexture(RealOpenGLEnums.GL_TEXTURE_2D, tex);
			
			ctx.texParameteri(RealOpenGLEnums.GL_TEXTURE_2D, RealOpenGLEnums.GL_TEXTURE_WRAP_S, RealOpenGLEnums.GL_REPEAT);
			ctx.texParameteri(RealOpenGLEnums.GL_TEXTURE_2D, RealOpenGLEnums.GL_TEXTURE_WRAP_T, RealOpenGLEnums.GL_REPEAT);
			ctx.texParameteri(RealOpenGLEnums.GL_TEXTURE_2D, RealOpenGLEnums.GL_TEXTURE_MIN_FILTER, RealOpenGLEnums.GL_NEAREST_MIPMAP_LINEAR);
			ctx.texParameteri(RealOpenGLEnums.GL_TEXTURE_2D, RealOpenGLEnums.GL_TEXTURE_MAG_FILTER, RealOpenGLEnums.GL_NEAREST);
			ctx.texParameterf(RealOpenGLEnums.GL_TEXTURE_2D, 0x000084FE, 16.0f);
			
			ctx.texImage2D(RealOpenGLEnums.GL_TEXTURE_2D, 0, RealOpenGLEnums.GL_RGBA, 4, 3, 0, RealOpenGLEnums.GL_RGBA, RealOpenGLEnums.GL_UNSIGNED_BYTE, pixels);
			ctx.generateMipmap(RealOpenGLEnums.GL_TEXTURE_2D);
			...
			ctx.bindBuffer(RealOpenGLEnums.GL_ARRAY_BUFFER, buf);
			ctx.bufferData(RealOpenGLEnums.GL_ARRAY_BUFFER, verts, RealOpenGLEnums.GL_STATIC_DRAW);
			...
			ctx.vertexAttribPointer(0, 2, RealOpenGLEnums.GL_FLOAT, false, 8, 0);
			
			ctx.viewport(0, 0, 400, 300);
			ctx.drawArrays(RealOpenGLEnums.GL_TRIANGLES, 0, 6);
			...
			ctx.readPixels(175, 150, 1, 1, RealOpenGLEnums.GL_RGBA, RealOpenGLEnums.GL_UNSIGNED_BYTE, readPx);
