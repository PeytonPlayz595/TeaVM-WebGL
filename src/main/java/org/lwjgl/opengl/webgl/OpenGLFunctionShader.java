...
public class OpenGLFunctionShader {
	...
	private static final OpenGLFunctionShader[] instances = new OpenGLFunctionShader[128];
	...
	public static OpenGLFunctionShader instance(int i) {
		...
		if (s == null) {
			...
			s = new OpenGLFunctionShader(i, CC_a_color, CC_a_normal, CC_a_texture0, CC_lighting, CC_fog, CC_alphatest, CC_unit0);
			instances[i] = s;
		}
		return s;
	}
	...
	private OpenGLFunctionShader(int j, boolean CC_a_color, boolean CC_a_normal, boolean CC_a_texture0,
	...
	public void setupArrayForProgram() {
		...
		_wglVertexAttribPointer(a_position, 3, WebGL._wGL_FLOAT, false, 28, 0);
		if (enable_texture0) {
			_wglEnableVertexAttribArray(a_texture0);
			_wglVertexAttribPointer(a_texture0, 2, WebGL._wGL_FLOAT, false, 28, 12);
		}
		if (enable_color) {
			_wglEnableVertexAttribArray(a_color);
			_wglVertexAttribPointer(a_color, 4, WebGL._wGL_UNSIGNED_BYTE, true, 28, 20);
		}
		if (enable_normal) {
			_wglEnableVertexAttribArray(a_normal);
			_wglVertexAttribPointer(a_normal, 4, WebGL._wGL_UNSIGNED_BYTE, true, 28, 24);
		}
	}
